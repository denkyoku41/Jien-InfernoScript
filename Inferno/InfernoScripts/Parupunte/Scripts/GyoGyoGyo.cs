...
{
    [ParupunteConfigAttribute("ギョギョギョｗｗｗｗ", "　ｗｗｗ　")]
    [ParupunteIsono("ぎょぎょ")]
    //[ParupunteDebug(true)]
    internal class Gyogyogyo : ParupunteScript
    {
        ...
        public Gyogyogyo(ParupunteCore core, ParupunteConfigElement element) : base(core, element)
        {
            SetUpSound();
        }
        ...
        public override void OnStart()
        {
            ...
            var animDict = "creatures@cat@move";
            ...
            foreach (var ped in core.CachedPeds.Where(x => x.IsSafeExist() && x.IsInRangeOf(core.PlayerPed.Position, 100) && x.IsAlive))
            {
                ...
                StartCoroutine(GyoGyoCoroutine(ped));
            }
        ...
        private IEnumerable<object> GyoGyoCoroutine(Ped ped)
        {
            while (!ReduceCounter.IsCompleted)
            {
                ...
                Function.Call(Hash.APPLY_FORCE_TO_ENTITY, ped, hp.X * 1, hp.Y * 1, hp.Z * 1, 0, 0, 0, 1, false,
                    true, true, true, true);
                Function.Call(Hash.TASK_PLAY_ANIM, ped, "creatures@cat@move", "gallop_turn_l", 8.0, -8.0, -1, 0, 0.5
                , 0, 0, 0);
                WaitForSeconds(2.0f);
                yield return null;
            }
        ...
        private void SetUpSound()
        {
            ...
            var setupWav = filePaths.FirstOrDefault(x => x.Contains("Gyogyogyo.wav"));
            if (setupWav != null)
            {
