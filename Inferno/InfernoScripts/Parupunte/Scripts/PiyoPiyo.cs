...
{
    [ParupunteConfigAttribute("ピヨピヨ", "　おわり　")]
    [ParupunteIsono("ぴよぴよ")]
    //[ParupunteDebug(true)]
    internal class Piyopiyo : ParupunteScript
    {
        ...
        public Piyopiyo(ParupunteCore core, ParupunteConfigElement element) : base(core, element)
        {
            SetUpSound();
        }
        ...
        public override void OnStart()
        {
            ...
            var animDict = "misschinese2_bank5";
            ...
            foreach (var ped in core.CachedPeds.Where(x => x.IsSafeExist() && x.IsInRangeOf(core.PlayerPed.Position, 100) || x.IsDead && x.IsAlive))
            {
                ...
                StartCoroutine(PiyoCoroutine(ped));
            }
        ...
        private IEnumerable<object> PiyoCoroutine(Ped ped)
        {
            ...
            while (!ReduceCounter.IsCompleted)
            {
                ...
                Function.Call(Hash.APPLY_FORCE_TO_ENTITY, ped, hp.X * 1, hp.Y * 1, hp.Z * 1, hp.X, hp.Y, hp.Z, 1, false,
                    true, true, true, true);
                Function.Call(Hash.TASK_PLAY_ANIM, ped, "ped", "handsup_exit", 9.0, 0, -1, 0, 0
                , 0, 0, 0);
        ...
        private void SetUpSound()
        {
            ...
            var setupWav = filePaths.FirstOrDefault(x => x.Contains("PiyoPiyo.wav"));
            if (setupWav != null)
            {
