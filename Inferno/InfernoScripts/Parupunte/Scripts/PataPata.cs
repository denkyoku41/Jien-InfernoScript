...
{
    [ParupunteConfigAttribute("パタパタ", "　おわり　")]
    [ParupunteIsono("ぱたぱた")]
    //[ParupunteDebug(true)]
    internal class Patapata : ParupunteScript
    {
        ...
        public Patapata(ParupunteCore core, ParupunteConfigElement element) : base(core, element)
        {
            SetUpSound();
        }
        ...
        public override void OnStart()
        {
            ...
            foreach (var ped in core.CachedPeds.Where(x => x.IsSafeExist() && x.IsInRangeOf(core.PlayerPed.Position, 100) && x.IsAlive))
            {
                ...
                StartCoroutine(FlyCoroutine(ped));
            }
        ...
        private void SetUpSound()
        {
            ...
            var setupWav = filePaths.FirstOrDefault(x => x.Contains("Patapata.wav"));
            if (setupWav != null)
            {
