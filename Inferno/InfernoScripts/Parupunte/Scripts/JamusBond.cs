...
{
    [ParupunteConfigAttribute("007　ロスサントスより愛をこめて", "　おわり　")]
    [ParupunteIsono("ぼんど")]
    //[ParupunteDebug(true)]
    internal class JamusBond : ParupunteScript
    {
        ...
        public JamusBond(ParupunteCore core, ParupunteConfigElement element) : base(core, element)
        {
            SetUpSound();
        }
        ...
        private IEnumerable<object> PlayCoroutine(Ped ped)
        {
            ...
            while (!ReduceCounter.IsCompleted)
            {
                ...
                Function.Call(Hash.APPLY_FORCE_TO_ENTITY, ped, hp.X * 1, hp.Y * 1, hp.Z * 1, hp.X, hp.Y, hp.Z, 1, false,
                    true, true, true, true);
                Function.Call(Hash.TASK_PLAY_ANIM, ped, "misschinese2_bank5", "peds_shootcans_a", 9.0, 0, -1, 0, 0
                , 0, 0, 0);
        ...
        private void SetUpSound()
        {
            ...
            var setupWav = filePaths.FirstOrDefault(x => x.Contains("JamusBond.wav"));
            if (setupWav != null)
            {
